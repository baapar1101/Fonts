...
let Checkfield = (element ,type ,err) => {
    ...
    if (type === "phone" || type === "phone-forgot") {
        if (input.length < 11 || input.length > 11) {
            ...
            AccountError(err ,null ,"hide")
            IsChecked = true                
            type === "phone-forgot" ? CheckForgot('code-link' ,'btn-code' ,IsChecked):null
        }
    ...
    // CheckLogin('login-link' ,'btn-login' ,IsChecked ,IsPassChecked)
}


let CheckPostalCode = (element , err) => {
    let postalCode = $.getElementById(element).value
    if (postalCode.length < 10 || postalCode.length > 10) {
        AccountError(err ,'کد پستی شامل 10 رقم است','show')
        return false
    }
    else {
        AccountError(err ,'' ,'hide')
